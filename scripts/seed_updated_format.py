#!/usr/bin/env python3
"""
Updated DDragon data seeder for TFT Bible MongoDB
...
def transform_champion_data(raw_champions: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for key, raw_champion in raw_champions.items():
        ...
        # The Rust model has: name, cost, traits, image_url, splash_url, stats, and ability
        champion = {
            'name': raw_champion.get('name', ''),
            'cost': raw_champion.get('tier', 1),
            'traits': [],  # Will need to map from raw data (trait key names)
            ...
            # The Rust model expects a nested 'stats' object
            'stats': {
                'health': 800.0,  # Default value (would need to be sourced from championFull.json)
                'mana': 100.0, 
                'starting_mana': 50.0,  # This is the problematic field
                'armor': 20.0,
                'magic_resist': 20.0,
                'attack_damage': 50.0,
                'attack_speed': 0.6,
            ...
            # The Rust model expects a nested 'ability' object
            'ability': {
...
def seed_mongodb(data: Dict[str, Any], db):
    ...
    print(f"  Champions: {len(champions)} (with nested stats and ability objects)")
...
def main():
    ...
    print("\nConnecting to tft_champions_db MongoDB...")
